...
Example `T t = ts[0][0]`:: If we follow the `List.get` analogy, we must arrive at `t:*-HC-0.0:ts`,
with `0.0` rather than `0` because the hidden content type `T` is not known in the outer list.
