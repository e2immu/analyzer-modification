...
- -S- indicates that two variables point to the same object, in a static way (assignment in the code)
- -E- indicates that two variables point to the same object, computed, e.g., by means of an `@Identity` method or some other computation
- -D- indicates that the two objects share some accessible content (they are Dependent)
- -HC- indicates that the two objects do not share accessible content, but do share hidden content

Note:

1. -S- and -E- are different only from an implementation point of view.
2. -D- optionally takes linking info; -HC- links always take linking info.
...
- `-D-` indicates that two objects share mutable accessible content, and there is either no information about how their hidden content (if present) is shared, or they have no hidden content in common
- `0-D-1` indicates that two objects share mutable accessible content, and objects of the hidden content type with index 0 are shared with the other object, where the same (at least, assignable) type has index 1. A typical first example is List<T> sub = list.subList(0, 3).
The result of this action is sub `0-D-0` list.
...
`List<Mutable> sub = list.subList(0, 3)` results in sub `0M-D-0M`.

`Set<Map.Entry<Mutable, T>> entrySet = map.entrySet` results in `0.0M,0.1-D-0,1`.
...
`X x = listX.get(3);` results in x `*-HC-0` listX

`M m = listM.get(3);` results in m `*M-D-0M` listM.
...
Knowing that -D- and -HC- links can only be created through method calls and field or array access, we create different implementations for each of the situations.
...
When writing `Y y = x.y`, we link part of the hidden content of `x` to variable `y`: `x` links `0-HC-2` or `0-HC-\*` to `y`, where `0` is the index of the hidden content of type `Y` in `X`, and `2` or `*` is the hidden content of `Y` itself.

So after `Z z = x.zy`, `x` links `1-HC-*` to `Z`, or `Z` links `0-HC-1` to `X`; all links are reversible and bidirectional.

Writing `Z z = x.y.zx`, `x` still links `1-HC-*` to `Z` because we don't make any distinction between the different fields of the same hidden content type.

Writing `U u = z.y.u`, `x` now links `0.1-HC-*` to `U`, where the dotted notation `0.1` must be used because `U` has no explicit mention in the list of hidden content types of `X`.
...
* the _object_ links `-S-` to the variable `list`,
* the _return value_ links `*-HC-0` to the _object_ and
...
In this particular case, the variable `t` links `-S-` to the _return value_.

Combining the three links, we obtain that the variable `t` links `*-HC-0` to the variable `list`.

==== Parameter to object

Example: `list.subList(0, 5).add(t)`.

* _Parameter 0 of add_ links `-S-` to the variable `t`
* the _object of add_ links `0-HC-*` to _parameter 0_
* the _object of add_ links `-D-` to the _object of subList_
* the _object of subList_ links `-S-` to the variable `list`

There is essentially no difference between `rv-object` and `object-param` linking.
Eventually, we arrive at variable `list:0-HC-*:t`.
...
* _parameter 0_ links `-S-` to variable `list`
* _parameter 1_ links `-S-` to variable `t1`
* _parameter 2_ links `-S-` to variable `t2`

There is a formal link between formal parameter 0 and the varargs parameter 1: `p0:0-HC-0:v1`.
That translates in `p0:0-HC-\*:p1`, `p0:0-HC-*:p2`.

=== Array access

Example `T t = ts[0];`:: Note that there cannot be a difference between array access and `List.get(index)`.
So we must arrive at `t:*-HC-0:ts`.

Example `T t = ts[0][0]`:: If we follow the `List.get` analogy, we must arrive at `t:*-HC-0.0:ts`, with `0.0` rather than `0` because the hidden content type `T` is not known in the outer list.

=== Functional parameters

What do we want in concrete examples?

Example `Stream<T> s = IntStream.range(0, 3).mapToObj(list::get)`:: should end up `s:0-HC-0:list`: the hidden content of the `list` is propagated to the hidden content of the stream `s`.
Can also be written as `Stream<T> s = IntStream.range(0, 3).mapToObj(index -> list.get(index))`

Example `list1.forEach(list2::add)`:: should end up `list1:0-HC-0:list2`: the hidden content of the first list is propagated to the second list.
This is easier seen as `list1.forEach(t -> list.add(t))`

* `forEach takes a `Consumer c`, with method `void accept(T t)`.
We write `list1:0-HC-0:c`, but what we intend is to iteratively link the hidden content of `list1` to the parameter of the `accept` method.
