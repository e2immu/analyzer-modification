...
public class TestLinkObjectRecursion extends CommonTest {
    ...
    public void test0() {
        ...
        MethodInfo prepend = LL.findUniqueMethod("prepend", 1);
        {
            ...
            assertEquals("0,2-4-0,2:ll, 0-4-*:t", vi0This.linkedVariables().toString());
            ...
            assertEquals("0-4-*:t, 0,2-4-0,2:this", vi0LL.linkedVariables().toString());
        }
        ...
        MethodInfo prepend2 = LL.findUniqueMethod("prepend2", 1);
        {
            ...
            assertEquals("0-4-*:t, 0,2-4-0,2:this", vi0Rv.linkedVariables().toString());
        }
    }
    ...
    public void test1() {
        ...
        MethodInfo prepend = LL.findUniqueMethod("prepend", 1);
        {
            ...
            VariableData vd0 = VariableDataImpl.of(s0);
            VariableInfo vi0This = vd0.variableInfo(thisVar);
            assertEquals("0,2-2-0,2|*-1:ll, 0-4-*:t", vi0This.linkedVariables().toString());
            VariableInfo vi0T = vd0.variableInfo(prepend.parameters().get(0));
            assertEquals("*-4-0:ll, *-4-0:this", vi0T.linkedVariables().toString());
            VariableInfo vi0LL = vd0.variableInfo("ll");
            ...
            // at the same time, because LL is mutable (we do not analyzer IMMUTABLE_TYPE yet), ll is linked to 'this':
            // only the hidden content type 0 is added as context/label.
            assertEquals("0-4-*:t, 0,2-2-0,2|1-*:this", vi0LL.linkedVariables().toString());
        }
        assertEquals("0-4-*:t, 0,2-2-0,2|1-*:this", prepend.analysis().getOrDefault(LINKED_VARIABLES_METHOD, EMPTY)
                .toString());
        ...
        MethodInfo prepend2 = LL.findUniqueMethod("prepend2", 1);
        {
            ...
            assertEquals("", vi0T.linkedVariables().toString());
            VariableInfo vi0Rv = vd0.variableInfo(prepend2.fullyQualifiedName());
            assertEquals("0-4-*:t, 0,2-2-0,2|1-*:this", vi0Rv.linkedVariables().toString());
        }
        assertEquals("0-4-*:t, 0,2-2-0,2|1-*:this", prepend2.analysis().getOrDefault(LINKED_VARIABLES_METHOD, EMPTY)
                .toString());
    }
